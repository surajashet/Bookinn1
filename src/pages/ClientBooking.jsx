...
export default function ClientBooking() {
  ...
  // Get room_id from URL query parameter (e.g., ?room=1)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        alert(`Booking confirmed! Total: ₹${priceDetails.totalPrice.toLocaleString("en-IN")}`);
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: "20px" }}>
              ...
              <input
                type="date"
                required
                min={booking.check_in_date || today}
            ...
            {priceDetails.nights > 0 && available !== null && (
              <div style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                  <span>₹{priceDetails.pricePerNight} x {priceDetails.nights} night{priceDetails.nights !== 1 ? 's' : ''}</span>
                  <span>₹{priceDetails.pricePerNight * priceDetails.nights}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontWeight: "bold", borderTop: "1px solid #E4DDD4", paddingTop: "8px", marginTop: "8px" }}>
                  <span>Total</span>
                  <span style={{ color: "#4A7C72", fontSize: "20px" }}>₹{priceDetails.totalPrice.toLocaleString("en-IN")}</span>
                </div>
              </div>
            )}
