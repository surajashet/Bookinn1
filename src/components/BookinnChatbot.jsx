...
import { processMessage } from "../../lib/intent";
...
export default function BookinnChatbot({ userId }) {
  ...
  // Fetch user info and bookings when chat opens
  useEffect(() => {
    if (isOpen && userId && !userData) {
      ...
      if (localUser.user_id === userId) {
        setUserData({ username: localUser.username });
        fetchUserBookings(userId);
      } else {
        fetch(`${import.meta.env.VITE_API_URL || "http://localhost:3001"}/api/users/${userId}`)
          .then((r) => r.json())
          .then((d) => {
            if (d.success) {
              setUserData(d.data);
  ...
  const fetchUserBookings = async (uid) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || "http://localhost:3001"}/api/bookings/user/${uid}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await response.json();
      if (data.success && data.data.length > 0) {
        // Format bookings for context
        const bookingsSummary = data.data.slice(0, 5).map(b => {
          return `Booking #${b.booking_id}: Room ${b.rooms?.room_number || b.room_id} (${b.rooms?.room_type || "Standard"}), Check-in: ${b.check_in_date}, Check-out: ${b.check_out_date}, Status: ${b.booking_status}, Guests: ${b.guests}`;
        }).join(" | ");
        setUserBookings(bookingsSummary);
      }
    } catch (error) {
  ...
  const sendMessage = async () => {
    ...
    const userContext = [];
    if (displayName) userContext.push(`user's name is ${displayName}`);
    if (userBookings) userContext.push(`recent bookings: ${userBookings}`);
    if (userContext.length === 0) userContext.push("no bookings yet");
    
    const contextString = userContext.join(". ");

    setTimeout(async () => {
      const { reply } = await processMessage(userInput, contextString);
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div
                      style={{
                        ...
                        border: "1px solid #EDE7DE", fontSize: 12, color: "#A09890",
                      }}
                    >
            ...
            <div style={{ display: "flex", alignItems: "flex-end", gap: 12 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: message.trim() && !isTyping ? ACCENT : "#EDE7DE",
                  color: message.trim() && !isTyping ? "#fff" : "#A09890",
                  cursor: message.trim() && !isTyping ? "pointer" : "not-allowed",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 16, flexShrink: 0, transition: "all 0.2s",
                }}
                onMouseEnter={(e) => {
                  if (message.trim() && !isTyping) e.currentTarget.style.background = ACCENT_L;
                }}
                onMouseLeave={(e) => {
                  if (message.trim() && !isTyping) e.currentTarget.style.background = ACCENT;
                }}
              >
                ➤
              </button>
